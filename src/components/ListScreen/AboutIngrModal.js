...
const DishViewItem = ({dish,onPress,what}) => {
    
    const ingredient = dish?.ingredients.find((ing) => ing.id == what.id)
    ...
    return(
        ...
        <TouchableOpacity activeOpacity={1} onPress={() => onPress()} style={styles.cardView}>
            <View style={{flexDirection:'row',alignItems:'center'}}>
                <FastImage style={{width:common.getLengthByIPhone7(80),height:common.getLengthByIPhone7(48),marginRight:common.getLengthByIPhone7(16),borderRadius:16}} 
                ...
                <View>
                    ...
                    <View style={{flexDirection:'row'}}>
                        <Text allowFontScaling={false} style={styles.itemSubtitle}>
                            {ingredient?.count == 0 ? '' : ingredient?.count + ' '}
                        </Text>
                        <Text allowFontScaling={false} style={styles.itemSubtitle}> 
                            {ingredient?.unit}
                        </Text>
                        {ingredient?.piece ? 
                        <Text allowFontScaling={false} style={styles.itemSubtitle}> 
                            {' '} (~{Math.ceil(ingredient?.count/ingredient?.piece)} {ingredient?.piece_unit})
                        </Text> : null}
                    </View>
                </View>                
            </View>
...
export const AboutIngrModal = observer(({modal, closeModal,onPress,dishes=[],ingredient}) => {

    const body = []

    for (let i = 0; i < dishes.length; i++) {
        body.push(
            <DishViewItem dish={dishes[i]} key={dishes[i]?.id} what={ingredient} onPress={() => onPress(dishes[i])} />
        ) 
    }
