...
export const IngredientItem = ({item,onPress = () => null,display='flex'}) => {
    ...
    const onSwipeValueChange = swipeData => {
        ...
        if(isBuyed == true) {
            if(!currentItem.is_buyed){
                ReactNativeHapticFeedback.trigger(Platform.OS == 'ios' ? 'impactHeavy' : 'impactMedium', options)
                // setBuyed(true,key)
            }
    ...
    return (
        <TouchableOpacity 
            ...
            <View ref={viewRef} 
                ...
                collapsable={false}
                onLayout={(e) => {
                    Platform.OS == 'android' ? setViewCoords(e.nativeEvent.layout.width) : null
                }}>
                <Animated.View
                collapsable={false}
                ...
                onPress={() => {
                    ...
                    setTimeout(() => {
                        ingredientHandler(currentItem.id,currentItem.is_buyed ? "notbuy" : "buy")
                        runInAction(() => onSwipeValueChange({"key": currentItem.id.toString(), "isBuyed": !currentItem.is_buyed}))                                
...
const styles = StyleSheet.create({
    ...
    buyText:{
        fontFamily:Platform.OS == 'ios' ? 'SF Pro Display' : 'SFProDisplay-Regular',fontSize:11,
        lineHeight:13,
        fontWeight:'500',
        color:'#FFF'
    },
