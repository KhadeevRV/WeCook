...
const ReceptDayScreen = observer(({navigation,route}) => {
    ...
    const handleDynamicLink = link => {
        // navigation.navigate('ReceptScreen',{rec:network.allDishes[0]})
        console.warn('asfnasfkaskfjaskfha',link)
        if(link && link.indexOf(Config.url) != -1){
            ...
            if(isRecp){
                ...
                let banner = network.banner1.type == 'menu_holiday' ? network.banner1 : 
                            network.banner2.type == 'menu_holiday' ? network.banner2 : null
    ...
    useEffect(() => {
        ...
        dynamicLinks()
        .getInitialLink()
        .then(link => handleDynamicLink(link?.url));
    }, [])
    ...
    return (
        ...
        <View style={{position:'absolute',zIndex:10,top:getStatusBarHeight(),alignSelf:'center',paddingTop:16,alignItems:'center'}}>
            <Text style={styles.title}>Рецепты дня</Text>
            <Text style={styles.subtitle}>{page + 1} из {network.dayDishes.length}</Text>
        </View>
        <FlatList
                showsVerticalScrollIndicator={false}
                showsHorizontalScrollIndicator={false}
                bounces={false} ref={scroll}
