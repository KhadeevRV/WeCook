...
const WelcomeScreen = observer(({navigation}) => {
  ...
  const nextScreen = () => {
    ...
    ampInstance.logEvent('name confirmed');
  ...
  return (
    <KeyboardAvoidingView
      ...
      <SafeAreaView />
      <SkipHeader
        skip={() => navigation.navigate(screen?.next_board, {name})}
        withBack={false}
      ...
      <ScrollView style={{backgroundColor: '#FFF', paddingTop: 8}}>
        <View style={{paddingHorizontal: 16}}>
          <Animated.View
            style={{opacity: fadeAnim, transform: [{translateY: marginAnim}]}}>
            <Text style={styles.title}>{network.strings?.Welcome}</Text>
            <Text style={styles.subtitle}>
              {network.strings?.WhatsYourName}
            </Text>
          </Animated.View>
        ...
        {!network.user?.email && (
          <View style={{flexDirection: 'row', alignSelf: 'center'}}>
            <Text style={styles.descr}>
              {network.strings?.AlreadyRegistered}
            </Text>
            <Text
              style={{
                ...styles.descr,
