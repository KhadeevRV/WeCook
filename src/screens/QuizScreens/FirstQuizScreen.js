...
import { StyleSheet, Text, View,SafeAreaView, Image, Platform,Animated, ImageBackground,InteractionManager } from 'react-native'
...
const FirstQuizScreen = observer(({navigation,route}) => {
    ...
    return (
        ...
        <FlatList
            ...
            <QuizItem title={item?.text} onPress={() => answerHandler(item)} icons={Array.isArray(screen?.answers[index].icon) ? item.icon : [item.icon]} key={item.id}/> 
            </Animated.View>
            }
        />
        </Animated.View>
