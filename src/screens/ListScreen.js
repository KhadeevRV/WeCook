...
const ListScreen = observer(({navigation}) => {
  ...
  for (let i = 0; i < network.listDishes.length; i++) {
    ...
    if (!item.disIngr) {
      for (let ind = 0; ind < item.ingredients.length; ind++) {
        ...
        sectionObj[ingredient.section] = isNotUndefined
          ? [...sectionObj[ingredient.section], ingredient]
          : [ingredient];
      }
    }
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    filteredArr = sectionObj[keys[i]].reduce((reducer, item) => {
      ...
      if (found) {
        ...
        newIngr.count = Math.ceil(
          parseFloat(item?.count * item?.persons) +
            parseFloat(newIngr?.initCount * newIngr?.persons),
        );
        newIngr.is_buyed = item?.is_buyed || newIngr?.is_buyed;
        newIngr.piece
          ? (newIngr.piece = Math.ceil(
              item?.piece * item?.persons +
                newIngr?.initPiece * newIngr?.persons,
            ))
          : null;
        ...
        newIngr.piece
          ? (newIngr.piece = Math.ceil(item?.piece * item?.persons))
          : null;
        ...
        style={{
          ...
          marginTop:
            i > 0
              ...
              : common.getLengthByIPhone7(25),
          display:
            filteredArr.findIndex(ing => !ing.is_buyed) != -1 ? 'flex' : 'none',
        }}
        key={keys[i]}>
        {keys[i]}
      </Text>,
    ...
    for (let y = 0; y < filteredArr.length; y++) {
      ...
      if (ingredient?.is_buyed) {
        buyedBody.push(
          <IngredientItem
            item={ingredient}
            onPress={() => openIngrModal(ingredient)}
            key={ingredient.id}
          />,
        );
      } else {
  ...
  const clearList = () => {
    Alert.alert(
      network?.strings?.ListClearTitle,
      network?.strings?.ListClearSubtitle,
      [
        {text: network?.strings?.DeleteButtonCancel},
        {
          text: network?.strings?.ListClearAccept,
          onPress: () => {
            listClear();
            navigation.goBack();
  ...
  const onShare = async () => {
    let row = [`${network?.strings?.ListShareTitle}${'\n'}`];
    for (let i = 0; i < keys.length; i++) {
      let items = [];
      // Преобразовываем массив, чтобы не было повторяющихся ингредиентов sectionObj - массив всех ингредиентов, где keys[i] - название секции
      ...
      for (let y = 0; y < filteredArr.length; y++) {
        ...
        if (!ingred.is_buyed) {
          items.push(
            `${ingred.name} - ${ingred.count == 0 ? '' : ingred.count} ${
              ingred.unit
            }`,
          );
        }
  ...
  const header = [
    <View style={styles.header} key={'listHeaderComponent'}>
      <TouchableOpacity
        activeOpacity={1}
        style={{
          position: 'absolute',
      ...
      <View style={{alignItems: 'center', alignSelf: 'center'}}>
        <Text style={styles.headerTitle}>{network?.strings?.ListTitle}</Text>
        <Text style={styles.headerSubitle}>
          ...
          , {ingredientsCount}
          {' ' +
            common.declOfNum(ingredientsCount, [
              network?.strings?.Product,
              network?.strings?.Products,
              network?.strings?.Products2,
            ])}
        </Text>
      </View>
      <View
        ...
        <TouchableOpacity
          activeOpacity={1}
          style={{paddingLeft: 14, paddingVertical: 9, paddingRight: 16}}
          onPress={() => onShare()}>
          <Image
      ...
      <ScrollView
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <ListItem
              recept={item}
              eyeHandler={status => eyeHandler(status, index)}
          ...
          <View style={{marginTop: 40}}>
            <View style={{paddingHorizontal: 16}}>
              <Text
                ...
                {network?.strings?.BuyedList}
              </Text>
              <Text
                style={{
                  ...styles.headerSubitle,
                  marginTop: 16,
                  marginBottom: 16,
                }}>
                {network?.strings?.BuyedListDescr}
              </Text>
            </View>
            {buyedBody}
          </View>
