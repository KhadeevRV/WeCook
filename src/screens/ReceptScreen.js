  ...
  for (let i = 0; i < step?.ingredients_used.length; i++) {
    ...
    subtitleView.push(
      <Text style={styles.stepSubtitle}>
        {ingr?.name + ' '}
        {count * parseFloat(ingr.count) == 0
          ? ingr.unit
          : ingr?.piece
          ? Math.ceil(count * ingr?.count) +
            ' ' +
            ingr?.unit +
            `(~${Math.ceil((count * ingr?.count) / ingr?.piece)} ${
              ingr?.piece_unit
            })`
        ...
        <>
          {Platform.OS == 'ios' ? (
            <FastImage
              style={{
                width: '100%',
                height: common.getLengthByIPhone7(232),
                borderRadius: 16,
              }}
              source={{uri: step?.images?.big_webp}}
              borderRadius={16}
            />
          ) : (
            <Image
      ...
      <View
        ...
        <View
          style={{
            ...
            borderRadius: 15,
            zIndex: 100,
            left: common.getLengthByIPhone7(0) / 2 - 30,
            alignItems: 'center',
        ...
        <View style={{flexDirection: 'row'}}>{subtitleView}</View>
      </View>
    </View>
  );
};
  ...
  return (
    <View style={{marginTop: 8, maxWidth: common.getLengthByIPhone7(80)}}>
      <Image
        ...
        source={{uri: ingredient?.images?.small_webp}}
        borderRadius={16}
        resizeMethod="resize"
      />
      ...
      {count * parseFloat(ingredient.count) == 0 ? (
        ...
        <>
          {ingredient?.piece ? (
            <Text
              style={{...styles.infoText, marginTop: 3, textAlign: 'center'}}>
              {Math.ceil(count * ingredient?.count)} {ingredient?.unit} (~
              {Math.ceil((count * ingredient?.count) / ingredient?.piece)}{' '}
              {ingredient?.piece_unit})
            </Text>
          ) : (
            <Text
              style={{...styles.infoText, marginTop: 3, textAlign: 'center'}}>
              {Math.ceil(count * parseFloat(ingredient.count)) + ' '}
              {ingredient.unit}
            </Text>
          )}
        </>
...
const ReceptScreen = observer(({navigation, route}) => {
  ...
  const [persons, setPersons] = useState(
    currentRec?.persons ?? network.user?.persons,
  );
  ...
  for (let i = 0; i < currentRec?.steps?.length; i++) {
    steps.push(
      <Step
        step={currentRec?.steps[i]}
        key={currentRec?.steps[i].id}
        count={persons}
      />,
    );
  }
  ...
  for (let i = 0; i < currentRec?.ingredients?.length; i++) {
    ingredients.push(
      <Ingredient
        ingredient={currentRec?.ingredients[i]}
        count={persons}
        key={currentRec?.ingredients[i].id}
      />,
    );
    if (i + 1 == currentRec?.ingredients?.length && (i + 1) % 4 != 0) {
      ...
      for (let i = 0; i < remains; i++) {
        ingredients.push(
          <View
            style={{width: common.getLengthByIPhone7(75)}}
            key={`remains${i}`}
          />,
        );
      }
  ...
  const onShare = async () => {
    try {
      ...
      Share.open(
        {
          message: network?.strings?.ShareRec + '\n' + shortUrl,
        },
        {
  ...
  const content = [
    <>
      <View
        style={{
          width: '100%',
          height: common.getLengthByIPhone7(520),
        ...
        <LinearGradient colors={['rgba(0, 0, 0, 0)', 'rgba(0, 0, 0, .3)']}>
          <View
            style={{
              paddingTop: common.getLengthByIPhone7(101),
              paddingBottom: 10,
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              />
              <Text style={{...styles.imageSubtitle, marginRight: 16}}>
                {currentRec?.eating}
              </Text>
              <Image
                style={{width: 17, height: 17, marginRight: 4}}
                source={require('../../assets/icons/clock.png')}
              />
              <Text style={{...styles.imageSubtitle, marginRight: 14}}>
                {currentRec?.cook_time} {network.strings?.minutes}
              </Text>
              {currentRec?.labels?.keto ? (
                <>
                  ...
                  />
                  <Text style={{...styles.imageSubtitle, color: '#D7FF95'}}>
                ...
                <>
                  ...
                  />
                  <Text
                    style={{...styles.imageSubtitle, color: Colors.greenColor}}>
                ...
                <>
                  ...
                  />
                  <Text style={{...styles.imageSubtitle, color: '#FFF495'}}>
        ...
        <FastImage
          ...
          source={{uri: currentRec?.images?.big_webp}}
          key={currentRec?.images?.big_webp}
          borderTopLeftRadius={24}
          borderTopRightRadius={24}
        />
      </View>
      <View
        ...
        <View
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              isInFavor
                ? network.deleteFromFavor(currentRec)
                : network.addToFavor(currentRec, navigation)
            }>
            <Image
              style={{width: 20, height: 19, marginRight: 15}}
              source={
                isInFavor
            ...
            <Text style={styles.subText}>
              {isInFavor
                ? network.strings?.InFavourites
                : network.strings?.AddToFavourites}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => onShare()}
            ...
            />
            <Text style={styles.subText}>{network.strings?.Share}</Text>
          </TouchableOpacity>
        </View>
        <View
          style={{
          ...
          <View
            style={{
              alignItems: 'center',
              width: (common.getLengthByIPhone7() - 32) / 4,
              borderRightWidth: 0.5,
              borderColor: '#EAEAEA',
            }}>
            <Text style={styles.bjuTitle}>{network.strings?.KCAL}</Text>
          ...
          <View
            ...
            <Text style={styles.bjuTitle}>{network.strings?.PROTEIN}</Text>
            <Text style={styles.bjuValue}>
              {currentRec?.protein} {network.strings?.grammsshortage}
            </Text>
          </View>
          {/* <View style={{width:0.5,height:'100%',backgroundColor:'#EAEAEA'}} /> */}
          <View
            ...
            <Text style={styles.bjuTitle}>{network.strings?.FAT}</Text>
            <Text style={styles.bjuValue}>
              {currentRec?.fats} {network.strings?.grammsshortage}
            </Text>
          </View>
          {/* <View style={{width:0.5,height:'100%',backgroundColor:'#EAEAEA'}} /> */}
          <View
            ...
            <Text style={styles.bjuTitle}>
              {network.strings?.CARBOHYDRATES}
            </Text>
            <Text style={styles.bjuValue}>
              {currentRec?.carbohydrates} {network.strings?.grammsshortage}
            </Text>
          </View>
        </View>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          />
          <Text style={styles.infoText}>{network.strings?.For100}</Text>
        </View>
        <Text style={{...styles.subtitle, marginTop: 35, marginBottom: 17}}>
          {network.strings?.RecipePersons}
        </Text>
        <View
          style={{
            shadowColor: '#000',
          ...
          <View style={styles.personsBar}>
            <TouchableHighlight
              onPress={() =>
                persons > 1 ? changeRecPersons(persons - 1) : null
              }
              style={{
            ...
            <TouchableHighlight
              onPress={() =>
                persons < 9 ? changeRecPersons(persons + 1) : null
              }
              style={{
                padding: common.getLengthByIPhone7(16),
                borderRadius: 17,
  ...
  useEffect(() => {
    const onFocus = navigation.addListener('focus', () => {
      ...
      ampInstance.logEvent('recipe opened', {recipe_id: currentRec?.id});
      if (Platform.OS == 'android') {
        fromDays
          ? StatusBar.setBackgroundColor('transparent', true)
          : StatusBar.setBackgroundColor('rgba(0,0,0,.4)', true);
      ...
      <TouchableHighlight
        ...
        <>
          ...
          <Image
            ...
            style={{width: 10, height: 10, tintColor: Colors.textColor}}
          />
        </>
      </TouchableHighlight>
    );
  ...
  const renderList = () => {
    if (isInList) {
      return (
        <TouchableHighlight
          ...
          onPress={() => network.deleteFromList(currentRec)}
    ...
    return (
      <TouchableHighlight
        ...
        onPress={() => network.addToList(currentRec)}
        underlayColor={Colors.underLayYellow}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <Image
    ...
    if (isInBasket) {
      return (
        <TouchableHighlight
          ...
          onPress={() =>
            network.basketHandle(isInBasket, currentRec?.id, persons)
          }
          disabled={isLoading}
          underlayColor={Colors.underLayYellow}>
          {isLoading ? (
      ...
      <TouchableHighlight
        ...
        onPress={() => {
          ...
          if (isInBasket || !isUnavailable) {
            ...
            let recipe = {...currentRec, persons};
            setUnavailableRecipe(recipe);
    ...
    <>
      <Modalize
        ...
        modalHeight={
          ...
          Platform.select({ios: 20 + getStatusBarHeight(), android: 0})
        }
        withOverlay={false}
        childrenStyle={{borderTopLeftRadius: 24, borderTopRightRadius: 24}}
...
const styles = StyleSheet.create({
  ...
  ingredientTitle: {
    ...
    marginBottom: 1,
    color: Colors.textColor,
    maxWidth: common.getLengthByIPhone7(80),
    textAlign: 'center',
  },
  stepTitle: {
    fontFamily: Platform.select({
